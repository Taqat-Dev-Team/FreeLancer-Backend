<div class="modal fade" id="editFormatModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form id="editFormatForm" enctype="multipart/form-data">
            @csrf
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <input type="hidden" id="edit_format_id" name="id">
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.edit-format', function (e) {
            ...
            $.ajax({
                url: `/admin/service-file-format/${id}/show`, // Use your existing show route
                type: 'GET',
                success: function (response) {
                    // Populate the form fields
                    $('#edit_format_id').val(response.id);
                ...
                error: function (xhr) {
                    toastr.error('Error fetching Format details.');
                }
            });
        });
        ...
        $('#editFormatForm').on('submit', function (e) {
            ...
            $.ajax({
                url: `/admin/service-file-format/${id}`, // RESTful route for 'update'
                ...
                error: function (xhr) {
                    if (xhr.status === 422) { // Laravel validation errors
                        ...
                        $.each(errors, function (key, value) {
                            let inputField = $(`#editFormatForm [name="${key}"]`);
                            if (inputField.length === 0) {
                                let fieldName = key.replace('.', '_');
                                inputField = $(`#editFormatForm [name="${fieldName}"]`);
                            }
