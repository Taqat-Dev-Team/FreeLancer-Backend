<div class="modal fade" id="addFormatModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog ">

        <form id="addFormatForm" enctype="multipart/form-data">
            @csrf
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#category_id_add').select2({
            dropdownParent: $('#addFormatModal'),
            width: '100%'
        });
        // Clear form and validation errors when the modal is hidden
        $('#addFormatModal').on('hidden.bs.modal', function () {
            $('#addFormatForm')[0].reset(); // Reset form fields
        ...
        $('#addFormatForm').on('submit', function (e) {
            ...
            $.ajax({
                url: "{{ route('admin.management.serviceFileFormat.store') }}",
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
                success: function (response) {
                    ...
                    toastr.success(response.message || 'Format added successfully!');
                    $('#formats_table').DataTable().ajax.reload();
                    $('#addFormatForm')[0].reset();
                },
                error: function (xhr) {
                    if (xhr.status === 422) {
                        let errors = xhr.responseJSON.errors;
                        $.each(errors, function (key, value) {
                            let inputField = $('#addFormatForm').find(`[name="${key}"]`);
